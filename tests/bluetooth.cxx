...
int main() {
  bluetooth::rfcomm_host server_host{bluetooth::local};//({0,0,0,0,4,20});
  bluetooth::rfcomm_host client_host{bluetooth::local};//({0,0,0,0,0,69});

  auto scanner = bluetooth::get_scanner();
  auto v = scanner->scan();
  v.finalise(1s);
  auto res = *v.try_take();

  for (auto i : res)
    std::cout << c3::nu::hex_encode_data(i) << std::endl;

  ::exit(1);

  auto server = server_host.listen(0);
  auto ep = server->get_ep();

  auto client_c = client_host.connect(ep);
