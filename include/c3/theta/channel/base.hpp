#pragma once

#include <c3/nu/data.hpp>
#include <c3/nu/bits.hpp>
#include <c3/nu/concurrency/timeout.hpp>
#include <c3/nu/concurrency/cancellable.hpp>

#include <memory>

#include <functional>
#include <future>
#include <chrono>

//! All of these should throw a timed_out exception when a reasonable amount of time has passed
//! If the timeout is non-positive, an exception should be immediately thrown

namespace c3::theta {
  // Something that can chuck bits into and gather bits from some medium
  //
  // Buffering is recommended but not mandatory
  // Must timeout if a reasonable time has passed
  template<nu::n_bits_rep_t DoF>
  class medium {
  public:
    virtual void send_points(gsl::span<const nu::bit_datum<DoF>>) = 0;
    virtual void receive_points(gsl::span<nu::bit_datum<DoF>>) = 0;

  public:
    virtual ~medium() = default;
  };

  // A medium that can send and receive sequences of octets
  template<size_t MaxFrameSize>
  class link {
  public:
    virtual void send_frame(nu::data_const_ref b) = 0;

    virtual nu::cancellable<nu::data> receive_frame() = 0;
  public:
    virtual ~link() = default;
  };

  // A medium that can send and receive arbitarily long sequences of octets (up to 2^64 - 1)
  // but without message delimiters
  class stream_channel {
  public:
    virtual void send_data(nu::data_const_ref b) = 0;

    virtual nu::cancellable<size_t> receive_data(nu::data_ref b) = 0;

  public:
    virtual ~stream_channel() = default;
  };

  // A link that can send and receive arbitarily long sequences of octets (up to 2^64 - 1)
  // with message delimiters
  class channel {
  public:
    virtual void send_msg(nu::data_const_ref) = 0;

    virtual nu::cancellable<nu::data> receive_msg() = 0;

  public:
    virtual ~channel() = default;
  };

  // A secure channel
  class conversation : public channel {
  public:
    virtual ~conversation() = default;
  };

  template<size_t MaxFrameSize, typename BaseChannel, typename = std::is_base_of<channel, BaseChannel>>
  class owning_link : public link<MaxFrameSize> {
  public:
    BaseChannel base;

  public:
    virtual void send_frame(nu::data_const_ref b) override {
      base.send_msg(b);
    }

    virtual nu::cancellable<nu::data> receive_frame() override {
      return base.receive_msg();
    }

  public:
    template<typename... Args>
    owning_link(Args... args) : base{std::forward<Args>(args)...} {}
  };

  template<size_t MaxFrameSize>
  class non_owning_link : public link<MaxFrameSize> {
  public:
    channel* base;

  public:
    virtual void send_frame(nu::data_const_ref b) override {
      base->send_msg(b);
    }

    virtual nu::cancellable<nu::data> receive_frame() {
      return base->receive_msg();
    }

  public:
    non_owning_link(channel* base) : base{base} {}
  };
}
